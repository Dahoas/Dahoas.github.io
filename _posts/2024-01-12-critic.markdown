...
Recent work [[1]](#references) studies the ability of SOTA large language models to reliably refine incorrect answers to math problems without any external feedback. Correctly refining a solution draft `D` of a question `Q` is a difficult task, requiring the model to first decide *when* to refine and then *where* and *how* to refine. Most models struggle to even reliably decide when a draft should be refined, immediately limiting performance. This is in contrast to working mathematicans who are adept at identifying reasoning errors and gauging the expected outcome of proof strategies. Often, even if a mathematician is unable to precisely locate the source of an error, they are able to identify a downstream issue later on and backtrack from there. Thus, improving the ability of an LLM to act as a zero-shot verifier of reasoning traces is crucial for advanced understanding of mathematics. 
